...
class View:
    ...
    def build_gui(self, placeholder_image):
        ...
        self.btn_open_directory = ttk.Button(
            master=frm_directory, style=SUCCESS, text="Open"
        )
        ...
        self.ent_tags = ttk.Entry(
            master=frm_tags_box, width=999, xscrollcommand=srb_tags.set
        )
        ...
        self.ent_image_text = ttk.Entry(
            master=frm_image_text_box, width=999, xscrollcommand=srb_image_text.set
        )
    ...
    def show_files(self, files_in_db, files_not_in_db):
        ...
        for file_name in sorted(files_in_db) + sorted(files_not_in_db):
            label = ttk.Label(
                master=self.frm_file_list,
                text=file_name,
                bootstyle=SUCCESS if file_name in files_in_db else SECONDARY,
            )
            label.pack(anchor=W)
            label.bind(
                "<Button-1>",
                lambda event, new_file=file_name: self.change_active_file_command(
                    new_file
                ),
            )
    ...
    def set_image(self, image_path=None, image_name=None):
        ...
        tk_image = ImageTk.PhotoImage(
            image.resize((400, int((image.height / image.width * 400))))
        )
