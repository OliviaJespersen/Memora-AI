    ...
    def clean(self):
        ...
        for object in self.metadata:
            for key in ["image_text", "tags"]:
                if (not object in os.listdir(self.active_directory) or
                    not isinstance(self.metadata[object], dict) or 
                    not key in self.metadata[object] or 
                    not isinstance(self.metadata[object][key], str)):
                    remove_list += [object]
                    break
